...
class _HistoryAbsensiPageState extends State<HistoryAbsensiPage> {
  ...
  Future<void> _loadHistoryAbsensi() async {
    ...
    try {
      ...
      if (response['success'] == true) {
        ...
        setState(() {
          // Filter: HANYA ambil data yang ada presensi_masuk
          // Jangan tampilkan jadwal yang tidak ada presensinya
          _absensi = data.where((item) => item['presensi_masuk'] != null).map((
            ...
            // Ambil status dari presensi_masuk, BUKAN dari item['status']
            ...
            return {
              ...
              "hari": item['hari'],
              "status": status, // Status dari presensi_masuk
  ...
  String _getStatusDisplay(String status) {
    // Mapping status dari backend ke display text
    switch (status) {
      case "hadir":
      case "terlambat":
        return "Reguler";
  ...
  String _parseWaktu(String? waktu) {
    ...
    try {
      // Jika format ISO 8601 (2025-10-02T06:40:02.0000)
      if (waktu.contains('T')) {
  ...
  bool _isRegularStatus(String status) {
    // Reguler = hadir atau terlambat (yang ada presensi masuk/pulang)
